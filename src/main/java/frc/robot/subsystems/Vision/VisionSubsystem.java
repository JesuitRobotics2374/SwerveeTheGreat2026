...
public class VisionSubsystem extends SubsystemBase{
     ...
     * @param tagID - the ID of the tag to check, where -1 checks for any visible tag.
    ...
    public List<EstimatedRobotPose> getGlobalFieldPoses() {
        ...
        for (int i = 0; i < numCams; i++) { // Iterate through each camera
            ...
            allPoses.add(cameras[i].getGlobalFieldPose()); // Add the estimated robot pose from the camera to the list
        }

        return allPoses; // Return the list of all estimated robot poses
    }

    /**
     * Gets the pose of a specific tag relative to the robot, averaged across all cameras.
    ...
    public Pose3d getNearestTag() {
        ...
        for (Pose3d pose : tagPoses) { // Iterate through each tag pose to find the nearest one
            if (pose.getTranslation().getNorm() < nearestTag.getTranslation().getNorm()) { // If the current tag is closer than the nearest found so far
    ...
    public double getDistanceToTag(int tagID) {
        ...
        System.out.println("norm = " + tagPose.getTranslation().getNorm());
        System.out.println("calculated = " + Math.sqrt(tagPose.getX() * tagPose.getX() + tagPose.getY() * tagPose.getY() + tagPose.getZ() * tagPose.getZ()));
